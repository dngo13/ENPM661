        ...
        while q:
            ...
            for next_node, cost in move(node):
                if not check_in_obstacle(next_node):
                    if next_node[0] < (ws_width) and next_node[1] < (ws_height):
...
# Function to draw the exploration and optimal path, saves to video
def visual(path, img, visited, out):
    imgcopy = img.copy()  # Make copy of image
    # ast.literal.eval line converts the tuple to int for processing
    visited = [ast.literal_eval(x.strip()) for x in visited]
    # Draws visited nodes
    for i in visited:
        # To fix the out of bounds error, anything above 300 is set to 299
        if 300-i[1] >= 299:
            val = 299
        elif 300-i[1] < 298:
            val = 300-i[1]
...
def main():
    ...
    out = cv2.VideoWriter('Dijkstra.mp4', fourcc, 3000,
                          (400, 300), isColor=True)
    ...
    for i in shortest_path:
        ...
        file.write("-------------" + "\n")
    file.close()
    # Creates visited text file to store the exploration path taken
    ...
    # End of main, closes  everytyhing out
